...
void music(Buzzer *buzzer)
{
	...
	while ((buzzer->state == BUZ_STATE_ON))
	{
		buzzer->noteLengths = HAL_GetTick();
		if ((buzzer->noteLengths) % 500 == 0)
		{
			...
			__HAL_TIM_SET_AUTORELOAD(buzzer->pwmTimer,
					buzzer->noteFreqs[buzzer->noteIdx]);
			__HAL_TIM_SET_COMPARE(buzzer->pwmTimer, buzzer->channel,
					buzzer->noteFreqs[buzzer->noteIdx] / 2);
